...
class Conditional_Graph_Gen(nn.Module):
    ...
    def forward(self, g_batch: Batch, bp_batch: Batch, parent_batch: Batch = None):
        ...
        loss_adj_g = torch.tensor(0., device=self.device)
        ...
        if self.dist == 'mix_Bernouli':
            ...
            if to_train:
                ...
                for i_bp, _end in enumerate(aug_edge_end_ls):
                    ...
                    if self.gen_completion_bipart and ('NumLeaf' in self.gen_completion_bipart) and self.is_leaf_level:
                        ...
                        nonzero_ind = pred_edges_values.nonzero().squeeze(dim=1)
                        if self.gen_completion_bipart and ('BinLeaf' in self.gen_completion_bipart) and self.is_leaf_level:
