...
def get_augmented_all_biparts(graph_parent: Data2, to_joint_aug_bp, ord, level,
    ...
    for e_, bipart_sample in enumerate(graph_parent.child_bipart):
        ...
        if to_train:
            ...
            # edge weight check relaxed for spectral clustering coarsened graphs
            # if has_edge_weight:
...
class HigenData(object):
    ...
    def __init__(self, config, file_names, num_graphs, tag='train'):
        ...
        # Use regular list; forked DataLoader workers inherit via COW
        self.file_names = list(file_names)

        # Pre-load all pickle files into memory to avoid repeated disk I/O (esp. on NFS)
        self._cache = {}
        logger.info(f"Pre-loading {num_graphs} graph files into memory...")
        t0 = time.time()
        for i, fn in enumerate(file_names):
            fn_str = fn if isinstance(fn, str) else fn[0]
            self._cache[fn_str] = pickle.load(open(fn_str, 'rb'))
        logger.info(f"Pre-loaded {num_graphs} files in {time.time()-t0:.1f}s")
