...
try:
    ...
    # Newer torch_geometric releases already register relu by default.
    pass
...
def dict_to_edict(value):
    if isinstance(value, list):
        return [dict_to_edict(item) for item in value]
    if isinstance(value, dict):
        converted = {key: dict_to_edict(val) for key, val in value.items()}
...
def get_config(config_file, exp_dir=None, is_test=False,
    ...
    """
    config = dict_to_edict(yaml.safe_load(open(config_file, 'r')))
...
def edict2dict(edict_obj):
    ...
    for key, vals in edict_obj.items():
        if isinstance(vals, dict):
            ...
            dict_obj[key] = [edict2dict(item) if isinstance(item, dict) else item for item in vals]
        else:
            dict_obj[key] = vals
    return dict_obj
