...
def _default_graph_type_tag(graph_type: str) -> str:
    graph_type_tag = graph_type_tags.get(graph_type)
    if graph_type_tag is not None:
        return graph_type_tag

    sanitized = ''.join(ch for ch in str(graph_type).lower() if ch.isalnum())
    return sanitized[:3] if sanitized else 'cus'


def load_custom_graph_list(input_path: str) -> List[nx.Graph]:
    ...
    if isinstance(graphs, dict):
        ...
        elif all(key in graphs for key in ['train', 'val', 'test']):
            graphs = list(graphs['train']) + list(graphs['val']) + list(graphs['test'])
        else:
            raise ValueError(
                f'Unsupported custom graph dictionary format in {input_path}. '
                'Expected keys `graphs` or `train`/`val`/`test`.'
            )

    if not isinstance(graphs, list):
        raise TypeError(f'Expected a list of NetworkX graphs in {input_path}.')

    out_graphs = []
    for graph in graphs:
        if not isinstance(graph, nx.Graph):
            raise TypeError(f'Expected NetworkX graphs in {input_path}, got {type(graph)}.')
        graph = graph.copy()
    ...
    if len(out_graphs) == 0:
        raise ValueError(f'No non-empty graphs found in {input_path}.')
...
def get_multilevel_graph_dataset(
    ...
    custom_graphs_path = kwargs.get('raw_graphs_path', None)
    ...
    elif custom_graphs_path:
        graphs = load_custom_graph_list(custom_graphs_path)
        if kwargs.get('splice_out_inds', None) is None:
            if max_num_level <= 2:
                kwargs.update(
                    {
                        'splice_out_inds':
...
def get_hierarchical_graph(
    ...
    g_type_tag = _default_graph_type_tag(graph_type)
    hg_dataset = []
    for i, G_nx in enumerate(graphs):
