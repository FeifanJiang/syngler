...
def sort_graph_nodes(G, G_parent, order='BFS'):
    ...
    perm_list = list(perm)
    # Handle nodes not visited by DFS (disconnected components)
    if len(perm_list) < G.num_nodes:
        visited = set(perm_list)
        for n in range(G.num_nodes):
            if n not in visited:
                perm_list.append(n)
    perm = dict([(jj, ii) for ii, jj in enumerate(perm_list)])
    G.perm = perm
