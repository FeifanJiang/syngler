...
def best_spectral_partition(G: Graph, k_min, k_max):
    ...
    partition_best = None
    modularity_ls = []
    adj_mat = nx.to_numpy_matrix(G)  # compute once outside loop
    n = adj_mat.shape[0]

    # For large graphs, subsample K values and reduce n_init to speed up
    k_range = list(range(k_min, k_max + 1))
    if len(k_range) > 10:
        # Sample ~10 evenly spaced K values instead of all
        step = max(1, len(k_range) // 10)
        k_range = k_range[::step]
        if k_range[-1] != k_max:
            k_range.append(k_max)
    _n_init = 3 if n > 2000 else 10
    print(f'SC0: n={n}, searching K in {k_range} (n_init={_n_init})')

    for K in k_range:
        # Perform spectral clustering algorithm
        sc = SpectralClustering(K, affinity='precomputed', n_init=_n_init)
        sc.fit(adj_mat)
    ...
    print(f'modularity score for K={k_range}:  {modularity_ls}')
