...
class HiGeN_Runner(object):
    ...
    def __init__(self, config):
        ...
        if self.dataset_conf.is_overwrite_precompute:
            _graphs0 = get_multilevel_graph_dataset(
                ...
                raw_graphs_path=config.dataset.get('raw_graphs_path', None),
                splice_out_inds=config.dataset.get('splice_out_inds', None),
            )
    ...
    def train(self):
        ...
        _num_workers = max(self.train_conf.num_workers, 4)
        _pin_memory = torch.cuda.is_available()
        logger.info(f"DataLoader: num_workers={_num_workers}, pin_memory={_pin_memory}")
        ...
        for epoch in range(resume_epoch, self.train_conf.max_epoch):
            ...
            for inner_iter in range(len(train_loader) // self.num_gpus):
                ...
                time_load_ = time.time()
                data_batch = train_dataset.to_device(data_batch, device=self.device)
                time_todev_ = time.time()
                if self.verbose > 2:
                    print(f"++++ Time to load a batch: {time_load_ - time0_:.3f}s, to_device: {time_todev_ - time_load_:.3f}s")
                iter_count += 1

                # Log data pipeline timing periodically
                if iter_count <= 5 or iter_count % 50 == 0:
                    logger.info(f"TIMING iter {iter_count}: data_load={time_load_ - time0_:.3f}s, to_device={time_todev_ - time_load_:.3f}s")
                ...
                if self.verbose > 2:
                    print(f"++++ Time to do one iteration: {time_gpu_ - time0_:.3f}s")
                if iter_count <= 5 or iter_count % 50 == 0:
                    logger.info(f"TIMING iter {iter_count}: gpu_fwd_bwd={time_gpu_ - time0_:.3f}s")
                ...
                # torch.cuda.empty_cache()  # removed: forces GPU sync every iter, kills perf
            lr_scheduler.step()
