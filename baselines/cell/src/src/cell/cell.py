...
class Cell(object):
    ...
    def __init__(self, A, H, loss_fn=None, callbacks=[], device=None):
        self.num_edges = A.sum() / 2
        self.A_sparse = A
        self.device = torch.device(DEVICE if device is None else device)
        self.A = torch.tensor(A.toarray(), device=self.device, dtype=DTYPE)
        self.step = 1
        ...
        gamma = np.sqrt(2 / (N + H))
        self.W_down = (
            (gamma * torch.randn(N, H, device=self.device, dtype=DTYPE))
            .clone()
            .detach()
            .requires_grad_()
        )
        self.W_up = (
            (gamma * torch.randn(H, N, device=self.device, dtype=DTYPE))
            .clone()
            .detach()
            .requires_grad_()
        )
    ...
    def __call__(self):
        ...
        """
        return (
            torch.nn.functional.softmax(self.get_W(), dim=-1)
            .detach()
            .to("cpu")
            .numpy()
        )
